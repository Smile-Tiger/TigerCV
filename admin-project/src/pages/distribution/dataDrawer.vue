...
import { getAgentList, getAgentOrderList } from '@/apis/distribution'
...
const drawerTitle = computed(()=> drawerType.value === 'user' ? "推广人列表" : "推广订单列表")
...
const getData = async (p = currentPage.value) => {
  ...
  const res = drawerType.value === 'user' ? await getAgentList(p, params) : await getAgentOrderList(p, params)
  console.log(res)
