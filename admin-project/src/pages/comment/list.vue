...
const getData = async (p = currentPage.value) => {
  loading.value = true
  currentPage.value = p
  const res = await getGoodsCommentList(p, searchForm.value)
...
const handleStatusChange = async (status, row) => {
  ...
  await updateGoodsCommentStatus(row.id, status)
  toast('状态修改成功')
  getData(1)
...
const review = async (row) => {
  if(textarea.value === '') return toast('评价内容不能为空', 'error')
